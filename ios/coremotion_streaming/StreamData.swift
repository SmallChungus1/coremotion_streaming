...
//        var request = URLRequest(url: URL(string: "ws://192.168.1.227:8000/ws")!) //ws or wss for websockets with startscream.
    ...
    func writeCombinedMotionLocationData(motion_data: [String:Double], location_data: [String:Double], stream_key: String, user_mode: String){
        
        //get current time
        let cur_time = self.isoFormatter.string(from: Date())
        //combine 2 dicts
        let merged_dict = motion_data.merging(location_data) { (motionVals, locationVals) in
            return motionVals + locationVals
        }
        // Insert the stream_key and user_mode and current time alongside numeric fields
        var payload: [String: Any] = merged_dict
        payload["stream_key"] = stream_key
        payload["cur_time"] = cur_time
        payload["user_mode"] = user_mode
